...
// import avCoreDocumentRoutes from './routes/avCoreDocumentRoutes.js';
...
// app.use('/api', avCoreDocumentRoutes);
...
import attendanceRoutes, { core as attendanceCore } from "./routes/attendanceRoutes.js";
...
import leadReportRoutes from './routes/leadReportRoute.js';
...
const allowedOrigins = ['https://react3.myospaz.in', 'http://localhost:3000', 'http://localhost:5173'];
...
app.use(session({
    key: "avcore.sid",
    secret: process.env.SESSION_SECRETE || 'test',
...
cron.schedule('0 0 * * *', async () => {
  try {
    console.log('⏱ Running daily attendance generation job...');
    await attendanceCore.jobs.generateTodayAttendance();
  } catch (error) {
    console.error('Attendance generation cron error:', error.message);
  }
});
// Routes
...
app.use('/api/report', leadReportRoutes);

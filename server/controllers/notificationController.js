...
export const sendFollowUpReminders = async () => {
  try {
    ...
    for (const lead of leads) {
      ...
      console.log('[verify] Calling createAdminNotification for follow-up reminder', {
        leadId: lead.master_id,
        assignedTo: lead.assignedTo,
        title: 'Follow‑up Reminder (Employee)',
        link,
      });
      await createAdminNotification(
        'Follow‑up Reminder (Employee)',
        `${lead.assignedTo || 'An employee'} has a follow‑up with "${lead.name}" in 5 minutes (scheduled at ${lead.followup_time}).`,
        link
      );
      console.log('[verify] Finished createAdminNotification for follow-up reminder', {
        leadId: lead.master_id,
        assignedTo: lead.assignedTo,
      });
    }
...
export const createAdminNotification = async (title, message, link = null) => {
  try {
    console.log('[verify] createAdminNotification started', { title, link, message });
    const [admins] = await db.query(
      `SELECT user_id
       FROM users
       WHERE status = 'active'
         AND LOWER(TRIM(COALESCE(role, ''))) IN ('admin', 'sub_admin')`
    );

    console.log('[verify] createAdminNotification recipients lookup', {
    ...
    if (!admins?.length) {
      console.log('No active admin/sub-admin users found, skipping admin notification');
      return;
    }

    for (const admin of admins) {
      console.log(`[verify] Creating admin notification for user ID: ${admin.user_id}`);
      console.log('------ admin notification is creating -------');
      await db.query(
        `INSERT INTO notifications (user_id, title, message, link, is_read) VALUES (?, ?, ?, ?, 0)`,
        [admin.user_id, title, message, link]
      );
      console.log(`[verify] Admin notification inserted for user ID: ${admin.user_id}`);
    }

    console.log('[verify] createAdminNotification completed', {
      title,

...
//     -- CITY LOGIC: If area_id exists, use area_name, else use city from raw_data
//     CASE 
//       WHEN rd.area_id IS NOT NULL AND a.area_name IS NOT NULL THEN a.area_name
//       ELSE IFNULL(rd.city, 'Not Available')
//     END AS city,
    
//     -- Alternatively, if you want to check area_id > 0 (assuming area_id is auto_increment starting from 1)
//     -- CASE 
//     --   WHEN rd.area_id > 0 AND a.area_name IS NOT NULL THEN a.area_name
//     --   ELSE IFNULL(rd.city, 'Not Available')
//     -- END AS city,
...
//     -- AREA / CATEGORY / REFERENCE (keep area_name if needed for other purposes)
...
export const getAllRawData = async (req, res) => {
  try {
    // 1️⃣ Fetch raw_data + joins (including latest reassignment AND status tracking)
    const query = `
  SELECT 
    rd.master_id,
    ...
    IFNULL(rd.min_budget, 'Not Available') AS min_budget,
IFNULL(rd.max_budget, 'Not Available') AS max_budget,
    ...
    -- ============================================================
    -- DEMO STATUS from lead_status_tracking
    -- ============================================================
    lst_demo.status AS demo_status,
    lst_demo.assigned_to_name AS demo_assigned_to,
    COALESCE(lst_demo.updated_at, lst_demo.created_at) AS demo_date,

    -- ============================================================
    -- SITE VISIT STATUS from lead_status_tracking
    -- ============================================================
    lst_site.status AS site_visit_status,
    lst_site.assigned_to_name AS site_visit_assigned_to,
    COALESCE(lst_site.updated_at, lst_site.created_at) AS site_visit_date
  ...
  -- DEMO STATUS JOIN
  -- ============================================================
  LEFT JOIN lead_status_tracking lst_demo 
    ON rd.master_id = lst_demo.master_id 
    AND lst_demo.status_type = 'demo'

  -- ============================================================
  -- SITE VISIT STATUS JOIN
  -- ============================================================
  LEFT JOIN lead_status_tracking lst_site 
    ON rd.master_id = lst_site.master_id 
    AND lst_site.status_type = 'site_visit'
...
`;
    ...
    const formattedRows = rows.map(row => {
      ...
      return {
        ...
        // Ensure status fields are properly set with defaults
        demo_status: row.demo_status || 'pending',
        site_visit_status: row.site_visit_status || 'pending',
        
        // Also pass the assigned_to_name for display
        demo_assigned_to: row.demo_assigned_to || '',
        site_visit_assigned_to: row.site_visit_assigned_to || '',
...
export const getCompleteRawData1 = async (req, res) => {
  try {
    if (!req.session.user) {
      return res.status(401).json({ message: "Unauthorized: No session" });
    }
  ...
  try {
    ...
    try {
      await createNotification(currentUserId, currentUserTitle, currentUserMessage, currentUserLink);
       console.log('[verify] Calling createAdminNotification for duplicate contact', {
         leadId: lead.master_id,
         currentUserId,
         number,
       ...
       await createAdminNotification(
         ...
         `Employee (ID: ${currentUserId}) hit a duplicate contact "${number}" on lead "${lead.name}" (ID: ${lead.master_id}).`,
         currentUserLink
       );
       console.log('[verify] Finished createAdminNotification for duplicate contact', {
         leadId: lead.master_id,
         currentUserId,
         number,
       });
      ...
      if (userRows.length > 0) {
        ...
        try {
          await createNotification(workingUserId, workingUserTitle, workingUserMessage, workingUserLink);
           console.log('[verify] Calling createAdminNotification for working user duplicate notice', {
             leadId: lead.master_id,
             workingUserId,
             title: 'Duplicate Contact — Working User Notified',
           });
           await createAdminNotification(
             'Duplicate Contact — Working User Notified',
             `Employee (ID: ${workingUserId}) was notified about a duplicate contact. Original message: ${workingUserMessage}`,
             workingUserLink
           );
           console.log('[verify] Finished createAdminNotification for working user duplicate notice', {
             leadId: lead.master_id,
             workingUserId,
           });
        } catch (notifError) {
...
export const updateRawData2 = async (req, res) => {
  console.log("---notification controller conlling")
  try {
    const { master_id } = req.params;
      ...
      for (const user of users) {
        ...
        if (finalUserId) {
          try {
            ...
            console.log('[verify] Calling createNotification for reassignment', {
              masterId: master_id,
              finalUserId,
              leadName,
              finalLeadStage,
            ...
            await createNotification(finalUserId, title, message, link);
            console.log('[verify] Calling createAdminNotification for reassignment', {
              masterId: master_id,
              finalUserId,
              leadName,
              finalLeadStage,
              title: 'Lead Assigned to Employee',
            });
            await createAdminNotification(
              'Lead Assigned to Employee',
              `Employee (ID: ${finalUserId}) was assigned to "${leadName}" (Stage: ${finalLeadStage || 'Not Set'}).`,
              link
            );
            console.log('[verify] Finished createAdminNotification for reassignment', {
              masterId: master_id,
              finalUserId,
              leadName,
            });
...
export const updateRawData = async (req, res) => {
  ...
  try {
    ...
    const allowed = {
      ...
      // ✅ ADD THESE TWO FIELDS TO THE allowed OBJECT
      min_budget: min_budget !== undefined && min_budget !== "" ? parseFloat(min_budget) : null,
      max_budget: max_budget !== undefined && max_budget !== "" ? parseFloat(max_budget) : null,
    ...
    Object.entries(allowed).forEach(([k, v]) => {
      // Skip undefined values, but allow null and empty string for certain fields
      if (v !== undefined) {
        // For number fields, allow 0 as valid value
        if (['min_budget', 'max_budget', 'room_length', 'room_width', 'room_height'].includes(k)) {
          if (v !== null && v !== undefined) {
            updateFields.push(`${k} = ?`);
            values.push(v);
          }
        } else if (v !== "" && v !== null && v !== undefined) {
          // For other fields, skip empty strings and null
          updateFields.push(`${k} = ?`);
          values.push(v);
        } else if (v === "" || v === null) {
          // Set empty strings and null to NULL in database
          updateFields.push(`${k} = ?`);
          values.push(null);
        }
      }
...
export const uploadDocuments = async (req, res) => {
  try {
   ...
   if (fileType === "video") {
  // Move uploaded video to temp folder
  ...
  // Compress
  await compressVideo(file.tempFilePath, savePath);
...
// In your backend (e.g., dropLeadsController.js)
export const getDropLeadsFullData2 = async (req, res) => {
  try {
    if (!req.session.user) {
      return res.status(401).json({ message: "Unauthorized: No session" });
    }
...
export const getDropLeadsFullData = async (req, res) => {
  try {
    ...
    /* ================= ROLE FILTER FOR COUNT ================= */
    // ✅ FIX FOR TECHNICAL_HEAD - Treat like admin (no assignedTo filter)
    if (role === 'technical_head') {
      // Technical head sees all drop leads - no filter
    } 
    else if (isTelecallerLike(role)) {
      countQuery += ` AND lr.assignedTo = ?`;
      countParams.push(currentUserName);
...
IFNULL(rd.max_budget, 'Not Available') AS max_budget,
    ...
    // ✅ FIX FOR TECHNICAL_HEAD - Treat like admin (no assignedTo filter)
    if (role === 'technical_head') {
      // Technical head sees all drop leads - no assignedTo filter
    } 
    else if (isTelecallerLike(role)) {
      query += ` AND lr.assignedTo = ?`;
      params.push(currentUserName);
...
export const getClosedLeadsFullData3 = async (req, res) => {
  try {
    if (!req.session.user) {
      return res.status(401).json({ message: "Unauthorized: No session" });
    }
...
export const getClosedLeadsFullData = async (req, res) => {
  try {
    ...
    // ✅ FIX FOR TECHNICAL_HEAD - Treat like admin (no assignedTo filter)
    if (role === 'technical_head') {
      // Technical head sees all closed leads - no assignedTo filter
    } 
    else if (isTelecallerLike(role)) {
      countQuery += ` AND lr.assignedTo = ?`;
      countParams.push(currentUserName);
...
export const getUpcomingAssignedFullData1 = async (req, res) => {
  try {
    if (!req.session.user) {
      return res.status(401).json({ message: "Unauthorized: No session" });
    }
...
export const getUpcomingAssignedFullData = async (req, res) => {
  try {
    ...
    // ✅ FIX FOR TECHNICAL_HEAD - Treat like admin (no assignedTo filter)
    if (role === 'technical_head') {
      // Technical head sees all upcoming data like admin
      query += ` AND rd.status IN ('Assigned', 'Not Interested')`;
    } 
    else if (isTelecallerLike(role)) {
      query += ` AND lr.assignedTo = ?`;
...
export const getEmployeeDetailedReport = async (req, res) => {
  try {
    ...
    SELECT
        ...
        CASE 
            WHEN la.leadStage = 'Drop' THEN 'Drop'
            WHEN la.leadStage = 'loss' THEN 'Lost'
            ELSE NULL
        END AS drop_loss_type,
        CASE 
            WHEN la.leadStage = 'Drop' OR la.leadStage = 'loss' THEN 1 
            ELSE 0 
        END AS is_dropped_or_lost
    FROM latest_assignment la
    ...
    // Define all stages from your database enum
    const ALL_STAGES = [
      'Fresh Lead',
      'Cold Lead',
      'On Hold',
    ...
    rows.forEach(row => {
      ...
      // Track stage counts - initialize all stages with 0
      const stage = row.stage || 'Not Contacted';
      
      // Initialize all stages with 0 if not already set
      ALL_STAGES.forEach(s => {
        if (!(s in employee.stage_counts)) {
          employee.stage_counts[s] = 0;
        }
      });
      
      // Increment the actual stage count
      employee.stage_counts[stage] = (employee.stage_counts[stage] || 0) + 1;
    ...
    // Calculate converted count based on Closed Deal, Execution, Pre Execution
    const CONVERTED_STAGES = ['Closed Deal', 'Execution', 'Pre Execution'];
    
    result.forEach(emp => {
      emp.converted_count = CONVERTED_STAGES.reduce((sum, stage) => {
        return sum + (emp.stage_counts[stage] || 0);
      }, 0);
      emp.not_converted_count = emp.total_assigned - emp.converted_count - emp.drop_count - emp.loss_count;
    });
...
export const getAssignedMissTodaysLeadsFullData6 = async (req, res) => {
  try {
    if (!req.session.user) {
      return res.status(401).json({ message: "Unauthorized: No session" });
    }
...
export const getAssignedMissTodaysLeadsFullData = async (req, res) => {
  try {
    ...
    // Add city filter - Updated to use the city logic
    if (cities.length > 0) {
      countQuery += ` AND (
        CASE 
          WHEN rd.area_id IS NOT NULL AND a.area_name IS NOT NULL THEN a.area_name
    ...
    // ✅ FIX FOR TECHNICAL_HEAD - Role filter for count
    if (role === 'technical_head') {
      // Technical head sees all data - no assignedTo filter
      // They can also filter by users if provided in the filter
    } 
    else if (isTelecallerLike(role)) {
      countQuery += ` AND lr.assignedTo = ?`;
      countParams.push(currentUserName);
...
IFNULL(rd.max_budget, 'Not Available') AS max_budget,
    ...
    // ✅ FIX FOR TECHNICAL_HEAD - Role filter for leads query
    if (role === 'technical_head') {
      // Technical head sees all data - no assignedTo filter
      // They can still filter by users if provided in the filter
    } 
    else if (isTelecallerLike(role)) {
      leadsQuery += ` AND lr.assignedTo = ?`;
      leadsParams.push(currentUserName);
    ...
    // ✅ FIX FOR TECHNICAL_HEAD - Count queries role filter
    if (role === 'technical_head') {
      // Technical head sees all data - no assignedTo filter
    } 
    else if (isTelecallerLike(role)) {

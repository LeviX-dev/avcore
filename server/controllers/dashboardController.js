...
export const getLeadStageSummary = async (req, res) => {
  try {
    ...
    const knownStages = [
      'Fresh Lead','Cold Lead','On Hold','Positive Lead','Pre Site Visit',
      'Quotation Pending','Quotation Follow-up',
      'Post Site Visit','Demo','Projection List','Drop','Closed Deal'
    ];
    ...
    let query = `
      SELECT 
        r.master_id,
        r.lead_stage AS rawStage,
        ra.leadStage AS reassignStage,
        ra.assignedTo AS currentAssignee
      FROM raw_data r
    ...
    // Initialize all known stages at 0
    knownStages.forEach(stage => summary[stage] = 0);
    summary["Others"] = 0;

    for (let row of rows) {
      let stage = row.reassignStage || row.rawStage;

...
export const getDashboardLeadCounts = async (req, res) => {
  try {
    ...
    // Get latest reassignments (used for all role-filtered queries)
    const [latestReassignments] = await db.query(`
      SELECT master_id, MAX(id) as latest_id
      FROM reassignment
      GROUP BY master_id
    ...
    // Helper to check if user can see all data
    const canSeeAllData = isAdminLike(role) || isManagementLike(role);
    ...
    // ===== DROP COUNT =====
    let dropQuery = `
      SELECT COUNT(DISTINCT rd.master_id) AS drop_count
      FROM raw_data rd
      LEFT JOIN reassignment re ON rd.master_id = re.master_id
      WHERE rd.lead_stage IN ('Drop', 'lost')
    `;
    const dropParams = [];
    
    if (!canSeeAllData) {
      dropQuery += ` AND re.id IN (?) AND re.assignedTo = ?`;
      dropParams.push(latestReassignmentIds, currentUserName);
    }
    const [dropRows] = await db.query(dropQuery, dropParams);

    // ===== CLOSED COUNT =====
    let closedQuery = `
      SELECT COUNT(DISTINCT rd.master_id) AS closed_count
      FROM raw_data rd
      LEFT JOIN reassignment re ON rd.master_id = re.master_id
      WHERE rd.lead_stage IN (?, ?, ?)
    `;
    const closedParams = [...closedStages];
    
    if (!canSeeAllData) {
      closedQuery += ` AND re.id IN (?) AND re.assignedTo = ?`;
      closedParams.push(latestReassignmentIds, currentUserName);
    }
    const [closedRows] = await db.query(closedQuery, closedParams);

    // ===== PROJECTION COUNT WITH ROLE FILTER =====
    let projectionQuery = `
      SELECT COUNT(DISTINCT rd.master_id) AS projection_count
      FROM raw_data rd
      LEFT JOIN reassignment re ON rd.master_id = re.master_id
      WHERE rd.lead_stage = 'Projection List'
    `;
    const projectionParams = [];

    // Apply role filter - only show assigned projections for non-admin users
    if (!canSeeAllData) {
      projectionQuery += ` AND re.id IN (?) AND re.assignedTo = ?`;
      projectionParams.push(latestReassignmentIds, currentUserName);
    }
    const [projectionRows] = await db.query(projectionQuery, projectionParams);

    // ===== QUOTATION PENDING COUNT =====
    let quotationPendingQuery = `
      SELECT COUNT(*) AS quotation_pending_count
      FROM raw_data rd
      LEFT JOIN reassignment re ON rd.master_id = re.master_id
      WHERE rd.lead_stage = 'Quotation Pending'
    `;
    const quotationPendingParams = [];

    if (!canSeeAllData) {
      quotationPendingQuery += ` AND re.id IN (?) AND re.assignedTo = ?`;
      quotationPendingParams.push(latestReassignmentIds, currentUserName);
    }
    const [quotationPendingRows] = await db.query(quotationPendingQuery, quotationPendingParams);

    // ===== QUOTATION FOLLOW-UP COUNT =====
    let quotationFollowupQuery = `
      SELECT COUNT(*) AS quotation_followup_count
      FROM raw_data rd
      LEFT JOIN reassignment re ON rd.master_id = re.master_id
      WHERE re.id IN (?)
      AND rd.lead_stage = 'Quotation Follow-up'
    `;
    const quotationFollowupParams = [latestReassignmentIds];
    
    if (!canSeeAllData) {
      quotationFollowupQuery += ` AND re.assignedTo = ?`;
      quotationFollowupParams.push(currentUserName);
    }
    const [quotationFollowupRows] = await db.query(
      quotationFollowupQuery,
    ...
    if (!canSeeAllData && role !== 'technical_head') {
      tmQuery += ` AND re.assignedTo = ?`;
      tmParams.push(currentUserName);
    }
    const [tmRows] = await db.query(tmQuery, tmParams);
    ...
    return res.status(200).json({
      ...
      closed: closedRows[0]?.closed_count || 0,
      projection: projectionRows[0]?.projection_count || 0,
      quotation_pending: quotationPendingRows[0]?.quotation_pending_count || 0,
...
export const getLeadStageSummary = async (req, res) => {
  try {
    ...
    const query = `
      SELECT
        CASE
          WHEN lead_stage IN ('Drop', 'lost') THEN 'Drop'
          WHEN lead_stage IN ('Closed Deal', 'Execution', 'Pre Execution') THEN 'Closed'
          ELSE lead_stage
        END AS stage,
...
export const getCategorySummary = async (req, res) => {
  try {
    ...
    if (startDate && endDate) {
      dateCondition = 'AND DATE(rd.created_at) BETWEEN ? AND ?';
      queryParams = [startDate, endDate];
    } else {
      dateCondition = 'AND DATE(rd.created_at) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)';
    }
...
// OPTIMIZED AREA SUMMARY - Add indexes and limit
export const getAreaSummary = async (req, res) => {
  try {
    ...
    // OPTIMIZED: Added LIMIT and better indexing
    let query = `
    ...
    // Added LIMIT to reduce data volume
...
export const getExecutionDashboardCounts4 = async (req, res) => {
  try {
    const user = req.session.user;
...
export const getExecutionDashboardCounts = async (req, res) => {
  try {
    ...
    // Helper function to check if role should see all data
    const canSeeAllData = () => {
      return ['admin', 'sub_admin', 'technical_head', 'project_manager'].includes(role); // Added project_manager
    };

    // Helper function to check if role is project_manager specifically
    const isProjectManager = () => {
      return role === 'project_manager';
    };

    // Helper function to check if role is telecaller like (excluding project_manager)
    const isTelecallerLike = () => {
      const telecallerRoles = [
        'tele_caller',
        'digital_marketing',
    ...
    if (canSeeAllData()) {
      // Admin, sub_admin, technical_head, AND project_manager see all assignments
      const [assignedRows] = await db.query(`
        SELECT 
          u.user_id,
          u.name,
          ...
          COUNT(epum.id) AS total_assigned
        FROM execution_process_user_map epum
        LEFT JOIN users u ON u.user_id = epum.user_id
        GROUP BY epum.user_id
        ORDER BY total_assigned DESC
      `);
      assignedProcessUserWise = assignedRows;
      ...
      // Telecaller roles see only their own assignments
      const [assignedRows] = await db.query(`
        SELECT COUNT(*) AS total
        FROM execution_process_user_map epum
        WHERE epum.user_id = ?
      `, [userId]);
      assignedProcessCount = assignedRows[0]?.total || 0;
    }
    ...
    if (canSeeAllData()) {
      // Admin, sub_admin, technical_head, AND project_manager see all documents
      const [dailyRows] = await db.query(`
        SELECT 
          u.user_id,
          u.name,
      ...
      // Telecaller roles see only their own documents
      const [dailyRows] = await db.query(`
        SELECT COUNT(*) AS total
        FROM execution_documents ed
        WHERE ed.uploaded_by = ?
      `, [userId]);
      dailyOperationCount = dailyRows[0]?.total || 0;
    }

    // ================= GET ASSIGNED LEADS FOR PROJECT MANAGER =================
    let assignedLeadsList = [];
    if (isProjectManager()) {
      const [assignedLeads] = await db.query(`
        SELECT DISTINCT rd.master_id, rd.name, rd.lead_stage

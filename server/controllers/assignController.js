...
export const assignLeads = async (req, res) => {
  ...
  const { assignType, mode, assignDate, targetDate, remark, leadCount } =
    req.body;
  const userId = req.session.user.id;
  ...
  try {
    ...
    for (let i = 0; i < leadIds.length; i++) {
      ...
      console.log(
        '[verify] Calling createAdminNotification for lead assignment',
        {
          leadId: leadIds[i],
          assignedTo: tele.name,
          title: 'Lead Assigned to Employee',
        },
      );
      await createAdminNotification(
        'Lead Assigned to Employee',
        `Lead (ID: ${leadIds[i]}) has been assigned to ${
          tele.name || 'an employee'
        }.`,
        `/call`,
      );
      console.log(
        '[verify] Finished createAdminNotification for lead assignment',
        {
          leadId: leadIds[i],
          assignedTo: tele.name,
        },
...
export const getAvailableCatArea = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Failed to fetch available category/area data.' });
  }
};

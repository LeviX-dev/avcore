...
try {
  ...
  console.log(`Sending expense submission notification to employee (ID: ${employeeId}) for expense (ID: ${expenseId})`);
  await createNotification(employeeId, title, message, link);
  console.log(`Notification sent to employee (ID: ${employeeId}) for expense (ID: ${expenseId})`);
  console.log('[verify] Calling createAdminNotification for expense submission', {
    expenseId,
    employeeId,
    finalStatus,
  ...
  await createAdminNotification(
    ...
    `An expense of ₹${numericAmount.toLocaleString('en-IN')} was submitted by employee (ID: ${employeeId}) (Status: ${finalStatus}).`,
    link
  );
  console.log('[verify] Finished createAdminNotification for expense submission', {
    expenseId,
    employeeId,
    finalStatus,
  });
console.log(`Admin notification sent for expense (ID: ${expenseId}) submitted by employee (ID: ${employeeId})`);
} catch (notifError) {
...
try {
  ...
  if (employeeId) {
    ...
    await createNotification(employeeId, title, message, link);
      console.log('[verify] Calling createAdminNotification for draft approval', {
        expenseId: id,
        employeeId,
        isPartial,
        title: '📝 Draft Expense Approved',
      });
      await createAdminNotification(
        '📝 Draft Expense Approved',
        `Draft expense of ₹${Number(rows[0].amount).toLocaleString('en-IN')} for employee (ID: ${employeeId}) was approved${isPartial ? ` (partial: ₹${finalApprovedAmount.toLocaleString('en-IN')})` : ''}.`,
        link
      );
      console.log('[verify] Finished createAdminNotification for draft approval', {
        expenseId: id,
        employeeId,
        isPartial,
      });
...
try {
  ...
  if (employeeId) {
    ...
    const link = `/expense/${id}`;
    await createNotification(employeeId, title, message, link);
     console.log('[verify] Calling createAdminNotification for draft rejection', {
       expenseId: id,
       employeeId,
       title: '📝 Draft Expense Rejected',
     });
     await createAdminNotification(
       '📝 Draft Expense Rejected',
       `Draft expense of ₹${Number(rows[0].amount).toLocaleString('en-IN')} for employee (ID: ${employeeId}) was rejected. Reason: ${normalizeText(remark) || 'No reason provided'}.`,
       link
     );
     console.log('[verify] Finished createAdminNotification for draft rejection', {
       expenseId: id,
       employeeId,
     });
  }
...
try {
  ...
  await createAdminNotification(
  '💳 Draft Converted to Expense',
  `Employee (ID: ${expense.employee_id || expense.created_by}) converted a draft to an expense. ₹${amountToDeduct.toLocaleString('en-IN')} deducted from their wallet.`,
  link
);
} catch (notifError) {
...
try {
  ...
  await createAdminNotification(
  '✅ Expense Approved',
  `Expense of ₹${Number(expense.amount).toLocaleString('en-IN')} for employee (ID: ${expense.employee_id}) was approved${isPartial ? ` (partially: ₹${finalApprovedAmount.toLocaleString('en-IN')})` : ''}.`,
  link
);
} catch (notifError) {
...
try {
  ...
  await createAdminNotification(
    '❌ Expense Rejected',
    `Expense of ₹${Number(expense.amount).toLocaleString('en-IN')} for employee (ID: ${expense.employee_id}) was rejected. Reason: ${normalizeText(status_remark) || 'No reason provided'}.`,
    link
  );
} catch (notifError) {

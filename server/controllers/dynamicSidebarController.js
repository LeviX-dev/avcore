  ...
  try {
    ...
    const query = `
      ...
      AND (
        ...
        /* Parent menu if any child is allowed */
        EXISTS (
          SELECT 1
          FROM sidebar_menu child
          INNER JOIN role_menu_permission p
            ON p.menu_key = child.menu_key
          WHERE p.role = ?
            AND p.can_view = 1
            AND child.parent_key = m.menu_key
        )
...
export const saveRolePermissions = async (req, res) => {
  try {
    ...
    /* Insert / Update selected permissions */
    if (permissions.length > 0) {

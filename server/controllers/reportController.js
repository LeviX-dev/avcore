...
export const getOverviewReport = async (req, res) => {
  try {
    ...
    reassignments.forEach(r => {
      ...
      switch(stage) {
        ...
        case 'Demo':
          ...
          break;
        case 'Quotation Pending':
          reportMap[emp].quotation_pending += 1;
          break;
        case 'Quotation Follow-up':
          reportMap[emp].quotation_followup += 1;
          break;
        case 'Projection List':
          reportMap[emp].projection_list += 1;
          break;
        case 'Closed Deal':
        case 'Execution':
        case 'Pre Execution':
    ...
    // ─────────────────────────────────────────────────────────────
    // 6️⃣ Calculate current_leads (latest stage is not closed/dropped/lost)
    // ─────────────────────────────────────────────────────────────
    // Get latest reassignment per employee per lead
    const latestReassignments = {};
    reassignments.forEach(r => {
    ...
    Object.values(latestReassignments).forEach(r => {
      ...
      // If current stage is not closed, dropped, or lost, count as current lead
      const closedStages = ['Closed Deal', 'Execution', 'Pre Execution'];
      const dropStages = ['Drop', 'loss'];
...
export const getOverviewReportDetails = async (req, res) => {
  try {
    ...
    switch(stage) {
      ...
      case 'Quotation Pending':
        stageCondition = `r.leadStage = ?`;
        stageValues = ['Quotation Pending'];
        break;
      case 'Quotation Follow-up':
        stageCondition = `r.leadStage = ?`;
        stageValues = ['Quotation Follow-up'];
        break;
      case 'Projection List':
        stageCondition = `r.leadStage = ?`;
        stageValues = ['Projection List'];
        break;
      case 'Closed':
        stageCondition = `r.leadStage IN (?, ?, ?)`;
        stageValues = ['Closed Deal', 'Execution', 'Pre Execution'];

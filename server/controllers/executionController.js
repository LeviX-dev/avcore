...
export const getClosedLeadsDataExe6 = async (req, res) => {
  try {
...
export const getClosedLeadsDataExe = async (req, res) => {
  try {
    ...
    let countQuery = `
      ...
      AND EXISTS (
        SELECT 1 FROM execution_start es
        WHERE FIND_IN_SET(rd.master_id, es.lead_ids)
        AND es.status IN ('pending', 'in_progress', 'hold_by_client', 'hold_by_avcore')
      )
    `;
    ...
    /* ================= MAIN QUERY - FIXED PAGINATION ================= */
    // First, get the distinct master_ids with pagination
    let idsQuery = `
      SELECT DISTINCT rd.master_id
      FROM raw_data rd
      LEFT JOIN (
        SELECT r1.*, ROW_NUMBER() OVER (PARTITION BY master_id ORDER BY id DESC) rn
        FROM reassignment r1
    ...
    const idsParams = [];
    if (isTelecallerLike(role)) {
      idsQuery += ` AND lr.assignedTo = ?`;
      idsParams.push(currentUserName);
    }

    // Add ORDER BY and pagination to the IDs query
    idsQuery += ` ORDER BY master_id DESC LIMIT ? OFFSET ?`;
    ...
    const masterIds = idsResult.map(row => row.master_id);
    
    // Now get the full data for these specific master_ids
    let query = `
      SELECT
        rd.master_id,
        IFNULL(rd.name, '') AS name,
        ...
        lr.remark AS latest_remark,
        DATE(lr.reassignment_date) AS latest_reassignment_date,
        IFNULL(u.name, '') AS telecaller_name,
        u.user_id AS assigned_to_user_id,
        es.execution_id,
        es.schedule_name AS execution_schedule_name,
        es.start_date AS execution_start_date,
        ...
        (
          SELECT IFNULL(SUM(DISTINCT et.completion_percentage), 0)
          FROM execution_process_logs epl
          JOIN execution_type et ON et.type_id = epl.type_id
          WHERE epl.lead_id = rd.master_id AND epl.status = 'completed'
        ) AS total_completion_percentage
      FROM raw_data rd
      LEFT JOIN area a ON rd.area_id = a.area_id
      LEFT JOIN category c ON rd.cat_id = c.cat_id
      ...
      LEFT JOIN users u ON lr.assignedTo = u.name
      LEFT JOIN execution_start es ON FIND_IN_SET(rd.master_id, es.lead_ids)
      WHERE rd.master_id IN (${masterIds.map(() => '?').join(',')})
    `;

    const params = masterIds;
    ...
    const formattedRows = rows.map((row) => {
      ...
      return {
        ...
        execution_process_count: row.execution_process_count || 0,
        execution_created_at: cleanValue(row.execution_created_at),
        total_completion_percentage: row.total_completion_percentage || 0
      };
    });

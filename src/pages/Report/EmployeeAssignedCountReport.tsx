...
interface EmployeeReport {
  ...
  drop_count: number;
  loss_count: number;
  total_dropped_lost: number;
  stage_counts: { [key: string]: number };
  lead_details: any[];
}
...
interface LeadData {
  ...
  drop_loss_type?: string;
  drop_loss_date?: string | null;
  drop_loss_remark?: string | null;
  drop_loss_status?: string;
  is_dropped_or_lost?: number;
}

const PAGE_SIZE = 10;

// All possible stages from the database enum
const ALL_STAGES = [
  'Fresh Lead',
  'Cold Lead',
  'On Hold',
...
// Stages to exclude from the main columns (they have their own columns)
const EXCLUDED_STAGES = ['Drop', 'loss'];

// Stages that are considered "converted" for conversion rate calculation
...
const EmployeeDetailedReport = () => {
  ...
  // Get display stages (all stages except Drop and loss)
  const displayStages = useMemo(() => {
    return ALL_STAGES.filter(stage => !EXCLUDED_STAGES.includes(stage));
  }, []);
  ...
  const getStageColor = (stage: string) => {
    const colors: { [key: string]: string } = {
      ...
      'Cold Lead': 'bg-indigo-100 text-indigo-700',
      'On Hold': 'bg-yellow-100 text-yellow-700',
      'Positive Lead': 'bg-green-100 text-green-700',
      'Pre Site Visit': 'bg-teal-100 text-teal-700',
      'Quotation Pending': 'bg-orange-100 text-orange-700',
      'Quotation Created': 'bg-amber-100 text-amber-700',
      'Quotation Follow-up': 'bg-purple-100 text-purple-700',
      'Post Site Visit': 'bg-cyan-100 text-cyan-700',
      'Demo': 'bg-pink-100 text-pink-700',
      'Projection List': 'bg-emerald-100 text-emerald-700',
      'Drop': 'bg-red-100 text-red-700',
      'Closed Deal': 'bg-green-200 text-green-800',
      'Execution': 'bg-blue-200 text-blue-800',
      'Pre Execution': 'bg-indigo-200 text-indigo-800',
      'loss': 'bg-red-200 text-red-800'
    };
    return colors[stage] || 'bg-gray-100 text-gray-700';
  };

  // Calculate conversion rate based on Closed Deal, Execution, Pre Execution stages
  const getConversionRate = (employee: EmployeeReport) => {
    const convertedCount = CONVERTED_STAGES.reduce((sum, stage) => {
      return sum + (employee.stage_counts[stage] || 0);
    }, 0);
    
    return employee.total_assigned > 0 ? (convertedCount / employee.total_assigned) * 100 : 0;
  };
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    <th className="p-3 text-left font-medium text-gray-600 sticky left-0 bg-gray-50 z-10 min-w-[150px]">Employee</th>
                    <th className="p-3 text-left font-medium text-gray-600 min-w-[100px]">Role</th>
                    
                    {/* ORIGINAL COLUMNS - KEPT AS IS */}
                    <th className="p-3 text-center font-medium text-gray-600 min-w-[80px]">Total Assign</th>
                    <th className="p-3 text-center font-medium text-gray-600 min-w-[80px]">Converted</th>
                    <th className="p-3 text-center font-medium text-gray-600 min-w-[80px]">Pending</th>
                    <th className="p-3 text-center font-medium text-gray-600 min-w-[80px]">Drop</th>
                    <th className="p-3 text-center font-medium text-gray-600 min-w-[80px]">Lost</th>
                    
                    {/* NEW STAGE COLUMNS (except Drop and loss) - NOT CLICKABLE */}
                    {displayStages.map(stage => (
                      <th key={stage} className="p-3 text-center font-medium text-gray-600 min-w-[120px]">
                        <span className={`px-2 py-0.5 rounded text-xs ${getStageColor(stage)}`}>
                          {stage}
                        </span>
                      </th>
                    ))}
                    
                    <th className="p-3 text-center font-medium text-gray-600 min-w-[100px]">Conversion %</th>
                  </tr>
                </thead>
                ...
                <tbody className="divide-y divide-gray-100">
                  {paginatedData.length > 0 ? (
                    paginatedData.map((employee, index) => {
                      const conversionRate = getConversionRate(employee);
                      
                      return (
                        <tr key={index} className="hover:bg-gray-50">
                          <td className="p-3 font-medium sticky left-0 bg-white z-10">{employee.employee_name}</td>
                          ...
                          <td className="p-3 text-center">
                            <button
                              onClick={() => fetchEmployeeLeads(employee.employee_name, "drop")}
                              className="inline-flex items-center justify-center min-w-[2rem] px-2 py-1 rounded-md bg-red-100 text-red-700 hover:bg-red-200 transition-colors text-sm"
                              title={`View ${employee.drop_count || 0} dropped leads`}
                            >
                          ...
                          <td className="p-3 text-center">
                            <button
                              onClick={() => fetchEmployeeLeads(employee.employee_name, "lost")}
                              className="inline-flex items-center justify-center min-w-[2rem] px-2 py-1 rounded-md bg-purple-100 text-purple-700 hover:bg-purple-200 transition-colors text-sm"
                              title={`View ${employee.loss_count || 0} lost leads`}
                            >
                              {employee.loss_count || 0}
                            </button>
                          </td>
                          
                          {/* NEW STAGE COLUMNS - NOT CLICKABLE */}
                          {displayStages.map(stage => {
                            const count = employee.stage_counts[stage] || 0;
                            return (
                              <td key={stage} className="p-3 text-center">
                                <span
                                  className="inline-flex items-center justify-center min-w-[2rem] px-2 py-1 rounded-md text-sm font-medium"
                                  style={{
                                    backgroundColor: count > 0 ? '#e5e7eb' : 'transparent',
                    ...
                    <tr>
                      <td colSpan={7 + displayStages.length + 1} className="p-8 text-center text-gray-500">
                        No employees found
                      </td>
                    </tr>
                  )}
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50">
                        <tr>
                          ...
                          <th className="p-3 text-left font-medium text-gray-600">Stage</th>

...
//     try {
//       const res = await axios.get(BASE_URL + 'auth/check-session', {
//         withCredentials: true,
//       });

//       if (res.data.isAuthenticated) {
//         // Already logged in → go dashboard
//         handleLogin(true, res.data.role);
//         navigate('/dashboard');
//       }
//     } catch (err) {
...
//         handleLogin(true, data.user.role);
//       } else {
...
//     <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
//       <path
//         d="M17.94 17.94C16.23 19.19 14.18 20 12 20C7.5 20 3.5 17 1 12C2.17 9.61 3.89 7.61 6 6.17M9.9 4.24C10.6 4.08 11.29 4 12 4C16.5 4 20.5 7 23 12C22.34 13.34 21.52 14.54 20.57 15.57M1 1L23 23"
//         stroke="currentColor"
//         strokeWidth="2"
//       />
//     </svg>
...
//                     type="submit"
//                     // value="Sign In" 
//                     value={showOtp ? "Verify OTP" : "Sign In"}
...
function SignIn({ handleLogin }: { handleLogin: (auth: boolean, role: string) => void }) {
  ...
  // Combined submit handler with proper OTP flow
  const submitHandler = async (e) => {
    e.preventDefault();
    ...
    if (!showOtp) {
      ...
      try {
        ...
        if (response.status === 429) {
          // Rate limited
          setOtpCooldown(data.waitTime || 30);
        ...
        } else if (response.ok && data.otpRequired) {
    ...
    // STEP 2: Verify OTP
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          // Optional: Clear OTP input on failure
          setOtp('');
        }
      } catch (err) {
            ...
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              ...
              {otpCooldown > 0 && (
                <div className="mb-4 p-3 bg-yellow-100 border border-yellow-400 text-yellow-700 rounded">
                  Please wait {otpCooldown} seconds before requesting another OTP
                </div>
              )}
              ...
              <form onSubmit={submitHandler}>
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 text-black outline-none focus:border-primary dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary disabled:opacity-50"
                    />
                    <span className="absolute right-4 top-4">
                      <svg
                        className="fill-current"
                ...
                <div className="mb-5">
                  <input
                    type="submit"
                    value={showOtp ? (loading ? "Verifying..." : "Verify OTP") : (isLoading ? "Sending OTP..." : "Sign In")}
                    disabled={isLoading || loading || otpCooldown > 0}
                    className="w-full cursor-pointer rounded-lg border border-success bg-success p-4 text-white transition hover:bg-opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                  />
                </div>
              </form>
            </div>

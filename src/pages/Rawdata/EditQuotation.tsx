...
const EditQuotation = () => {
  ...
  /* ================= KIT EDIT FUNCTIONS ================= */
  // Enable kit edit mode for a specific category
  const enableKitEdit = (catIndex) => {
    const category = queuedCategories[catIndex];
    if (category && category.kit_id) {
      setEditingCategoryIndex(catIndex);
      setTempKitItems([...category.products]);
      setEditKitMode(true);
    } else {
      alert('This is not a kit item. Only kits can be edited.');
    }
  };
  ...
  // Open replace product popup
  const openReplaceProductPopup = (index) => {
    setReplaceProductIndex(index);
  ...
  const saveKitChanges = () => {
    if (editingCategoryIndex !== null) {
      const updatedCategories = [...queuedCategories];
      const kitQty = updatedCategories[editingCategoryIndex].kit_qty;
      
      // Update products in the category
      updatedCategories[editingCategoryIndex].products = [...tempKitItems];
      
      // Recalculate category total
      updatedCategories[editingCategoryIndex].category_total = tempKitItems.reduce(
        (sum, product) => sum + (product.model_qty * product.model_price * kitQty),
        0
      );
      
      setQueuedCategories(updatedCategories);
    }
    setEditKitMode(false);
    setTempKitItems([]);
  ...
  // Update product in temp kit items
...
• FABRIC STITCHING CHARGES WILL BE EXTRA AS PER THE DESIGN`
  ...
  const handleAddSingleProduct = () => {
    // If in replace mode for kit editing
    if (isReplaceMode && replaceProductIndex !== null && tempKitItems[replaceProductIndex]) {
      const updated = [...tempKitItems];
    ...
    <div>
      ...
      <div className="bg-white rounded shadow p-6 max-w-5xl mx-auto">
        ...
        <form onSubmit={handleSubmit} autoComplete="off" className="space-y-6 mt-6">
          ...
          {/* Quotation Items with Edit Functionality */}
          <div className="bg-gray-100 p-4 rounded">
            <h4 className="font-semibold mb-3">Quotation Items</h4>
            {queuedCategories.length === 0 ? (
              <div className="text-gray-500 italic text-sm">
                ...
                <div key={cIdx} className="bg-white border rounded p-3 mb-3">
                  ...
                  {editKitMode && editingCategoryIndex === cIdx && cat.kit_id ? (
                    <div>
                      <div className="flex justify-between items-center mb-3">
                        <h5 className="font-semibold text-blue-600">
                          Editing Kit: {cat.kit_name}
                        </h5>
                        <button
                          type="button"
                          onClick={cancelKitEdit}
                      ...
                      {tempKitItems.map((product, pIdx) => (
                        <div key={pIdx} className="bg-gray-50 border rounded p-3 mb-3">
                          <div className="font-semibold">{product.model}</div>
                          <div><b>Brand:</b> {product.brand_name}</div>
                          <div><b>Type:</b> {product.product_type_name}</div>
                          
                          {product.model_description && (
                            <div className="text-gray-600 text-xs mt-1">{product.model_description}</div>
                          )}
                          ...
                          <div className="flex gap-2 mt-2">
                            ...
                            <button
                              type="button"
                              onClick={() => openReplaceProductPopup(pIdx)}
                          ...
                          <div className="text-green-700 font-semibold mt-2 text-sm">
                            Subtotal: ₹{(product.model_qty * product.model_price * cat.kit_qty).toFixed(2)}
                          </div>
                        </div>
                      ))}
                    ...
                    <div>
                      ...
                      >
                        <div>
                          <b>{cat.kit_name || 'Single Product'}</b> | Qty: {cat.kit_qty} | Total: ₹{cat.category_total.toFixed(2)}
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`transition-transform ${expandedIndex === cIdx ? 'rotate-180' : ''}`}>
                            ▼
                          </span>
                        </div>
                      </div>
                      ...
                      {expandedIndex === cIdx && (
                        <div className="mt-3 border-t pt-3 space-y-3">
                          {cat.products.map((p, pIdx) => (
                            <div key={pIdx} className="bg-gray-50 border rounded p-3 text-sm">
                              <div className="font-semibold">{p.model}</div>
                              <div><b>Brand:</b> {p.brand_name}</div>
                              <div><b>Type:</b> {p.product_type_name}</div>
                              {p.model_description && (
                                <div className="text-gray-600 whitespace-pre-line mt-1">
                                  {p.model_description}
                                </div>
                              )}
                              <div className="mt-1"><b>Qty in Kit:</b> {p.model_qty}</div>
                              <div className="mt-1"><b>Unit Price:</b> ₹{p.model_price}</div>
                              <div className="mt-1"><b>Kit Qty:</b> {cat.kit_qty}</div>
                              <div className="text-green-700 font-semibold mt-2">
                                Total: ₹{(p.model_qty * p.model_price * cat.kit_qty).toFixed(2)}
                              </div>
                            </div>
                          ))}
                        </div>
          ...
          {openSingleProductPopup && (
            <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
              <div className="bg-white p-6 rounded shadow-lg w-[600px] max-w-full max-h-[90vh] overflow-y-auto">
                <h3 className="font-semibold text-lg mb-4">
                  {isReplaceMode ? 'Replace Product in Kit' : 'Add Single Product'}
                </h3>

...
const formatDateTime = (dateString: string | undefined): string => {
    ...
    try {
      ...
      const updateData: any = {
        ...
        budget_range: editingClient.budget_range, 
         ...
         // ✅ ADD THESE TWO LINES
      min_budget: editingClient.min_budget,
      max_budget: editingClient.max_budget,
  ...
  const handleInputChange = (
    ...
    if (
      ...
      // ✅ ADD THIS BLOCK - Handle the new budget fields
  if (['min_budget', 'max_budget'].includes(name)) {
    // Remove any non-digit characters
    const cleaned = value.replace(/[^0-9]/g, '');
    // If empty, keep as empty string, otherwise store as string
    processedValue = cleaned === '' ? '' : cleaned;
  }
...
<div>
  ...
  {/* ✅ ADD THESE TWO INPUT FIELDS AFTER THE DROPDOWN */}
  <div className="grid grid-cols-2 gap-2 mt-2">
    <div>
      <label className="block text-xs text-gray-600 dark:text-gray-400 font-medium mb-1">
        Minimum Budget (₹)
      </label>
      <input
        type="text"
        name="min_budget"
        value={editingClient.min_budget || ''}
        onChange={handleInputChange}
    ...
    <div>
      ...
      <input
        type="text"
        name="max_budget"
        value={editingClient.max_budget || ''}
        onChange={handleInputChange}

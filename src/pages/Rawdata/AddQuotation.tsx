...
const validateInstallments = () => {
  ...
  /* ---------------- KIT EDIT FUNCTIONS ---------------- */
  // Enable kit edit mode
  const enableKitEdit = () => {
    if (selectedKitData) {
      setTempKitItems([...selectedKitData.items]);
      setEditKitMode(true);
    }
  };
  ...
  const saveKitChanges = () => {
    const updatedKitData = { ...selectedKitData, items: tempKitItems };
    setSelectedKitData(updatedKitData);
...
const toggleExpand = (index) => {
  ...
  const handleAddSingleProduct = () => {
    // If in replace mode, replace the product in tempKitItems
    ...
    <p>
      ...
      <span className="text-gray-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 mt-9">
          ...
          {/* SELECTED KIT ITEMS WITH EDIT FUNCTIONALITY */}
          {selectedKitData && (
            <div className="bg-yellow-50 p-4 rounded">
              <div className="flex justify-between items-center mb-2">
                <h4 className="font-semibold">Kit Items</h4>
                {!editKitMode && (
                  <button
                    type="button"
                    onClick={enableKitEdit}
              ...
              {(editKitMode ? tempKitItems : selectedKitData.items).map((item, idx) => (
                <div key={item.kmap_id || idx} className="bg-white p-3 mb-2 border rounded text-sm">
                  <div>
                    <b>Product:</b> {item.product_type_name}
                  </div>
                  <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <input
                      type="number"
                      min={1}
                      value={editKitMode ? item.prod_qty : (item.prod_qty ?? item.qty)}
                      className="border px-2 py-1 rounded w-20"
                      onChange={(e) => {
                        if (editKitMode) {
                          const updated = [...tempKitItems];
                          updated[idx].prod_qty = Number(e.target.value);
                          setTempKitItems(updated);
                        } else {
                          updateItem(idx, 'prod_qty', e.target.value);
                        }
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <input
                      ...
                      min={0}
                      step="0.01"
                      value={editKitMode ? item.prod_price : (item.prod_price ?? item.price)}
                      className="border px-2 py-1 rounded w-28"
                      onChange={(e) => {
                        if (editKitMode) {
                          const updated = [...tempKitItems];
                          updated[idx].prod_price = Number(e.target.value);
                          setTempKitItems(updated);
                        } else {
                          updateItem(idx, 'prod_price', e.target.value);
                        }
                  ...
                  {editKitMode && (
                    <div className="mt-2">
                      ...
                      <button
                        type="button"
                        onClick={() => openReplaceProductPopup(idx)}
                        className="text-blue-500 text-sm font-bold hover:text-blue-700 ml-3"
                      >
                        🔄 Replace Product
                      </button>
                    </div>
                  ...
                  <div className="text-green-700 font-semibold mt-1">
                    Total: ₹{((item.prod_qty ?? item.qty) * (item.prod_price ?? item.price)).toFixed(2)}
                  </div>
              ...
              <div className="text-right font-semibold text-lg bg-green-100 text-green-800 px-4 py-2 rounded mt-3">
                Kit Cost (x{kitQty}): ₹
                {((editKitMode ? tempKitItems : selectedKitData.items).reduce(
                  (sum, item) => sum + (item.prod_qty ?? item.qty) * (item.prod_price ?? item.price),
                  0,
                ) * kitQty).toFixed(2)}
              </div>
            </div>
